...
.container { width:95%; max-width:700px; background:white; padding:20px; margin:20px auto; border-radius:12px; box-shadow:0 5px 20px rgba(0,0,0,0.05); }
...
<div class="navbar">
<div class="nav-left">📋 Login/Logout Logs</div>
...
<h3>Logs</h3>
<table>
<thead><tr><th>#</th><th>User</th><th>Login Time</th><th>Logout Time</th></tr></thead>
...
fetch(`api.php?action=getSessions&user_id=${userId}`)
.then(res=>res.json())
.then(data=>{
    let tbody=document.getElementById("logsTable");
    tbody.innerHTML="";
    data.forEach((s,i)=>{
        tbody.innerHTML+=`<tr>
        <td>${i+1}</td>
        <td>${localStorage.getItem("loggedInUser")}</td>
